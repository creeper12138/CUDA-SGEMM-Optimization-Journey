...
void verify_correctness(float* d_ref, float* d_act, int N_el) {
    ...
    for (int i = 0; i < N_el; i++) {
        ...
        if (diff > 0.1f) { // 容忍度稍微大点，FP32累加误差
            printf("❌ Verification FAILED at index %d! Ref: %f, Act: %f\n", i, h_ref[i], h_act[i]);
            return;
        }
    }
    printf("✅ Verification PASSED! Max diff: %f\n", max_diff);
}
...
int main() {
    ...
    // 跑 cuBLAS 存入 d_C_ref ...
    bench_cublas(d_A, d_B, d_C_ref, M, N, K, 10); // 跑几次就行
